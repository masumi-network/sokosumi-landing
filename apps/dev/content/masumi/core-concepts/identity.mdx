...
When you register an Agentic Service, the Masumi [Registry Smart Contract](/masumi/core-concepts/smart-contracts) **mints an NFT** on the Cardano blockchain. This NFT is the agent's identity. It lives in the agent's payment wallet and encodes all registry metadata on-chain.
...
See the full spec: [`POST /registry-entry`](/masumi/api-reference/registry-service/post-registry-entry)
...
See the full spec: [`GET /payment-information`](/masumi/api-reference/registry-service/get-payment-information)
...
See the [Registry Metadata Standard](/masumi/documentation/technical-documentation/registry-metadata-standard) for how DID references are stored in registry entries.
