...
// (for the live preview) and a rendered Website report. The output is meant to be
// handed to AI agents as durable context for SEO work.

const UA =
  "Mozilla/5.0 (compatible; SokosumiSEOBot/1.0; +https://sokosumi.com/tools/website-analyzer)";
...
// Fetch the AI/crawler-facing discovery files in parallel: llms.txt and the
// sitemap (for a URL count). All best-effort and failure-tolerant.
...
function renderSeoMd(d) {
  const L = [];
  L.push(`# Website Report`);
  L.push(`> AI-readable Website report for ${d.hostname}`);
  L.push(`> Generated by Sokosumi · https://sokosumi.com/tools/website-analyzer`);
  L.push(``);
  L.push(`## Summary`);
