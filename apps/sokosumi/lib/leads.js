...
//   SALES_NOTIFY_EMAIL where notifications go (default info@sokosumi.com)
...
async function storeLead(lead, source) {
  ...
  const res = await fetch(`${CMS_URL}/api/sales-inquiries`, {
    ...
    body: JSON.stringify({ ...lead, source, status: "new", notified: false }),
  });
  if (!res.ok) {
...
async function submitLead(body, source) {
  ...
  let id = null;
  try {
    id = await storeLead(v.lead, source);
  } catch (e) {
...
module.exports = { submitLead, rateLimited, validate };
