...
function tile(c, i) {
  const img = c.image
    ? `<span class="portrait${c.kind === "agent" ? " is-icon" : ""}"><img${shell.thumbSrc(c.image, 512, "src", 92)} alt="${attr(c.name)}" loading="lazy" decoding="async" /></span>`
...
async function profile(ctx) {
  ...
  return (
    ...
    `<div class="cw-hero">
      <div class="cw-portrait${c.kind === "agent" ? " is-icon" : ""}" data-reveal>${c.image ? `<img${shell.thumbSrc(c.image, 512, "src", 92)} alt="${attr(c.name)}" decoding="async" />` : ""}</div>
