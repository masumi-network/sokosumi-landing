...
|-------|-------------|-----|
| `sign_up_click` `{location, plan, seats}` | a "Sign Up" / "Get started" CTA is clicked | `data-analytics="sign_up_click"` (hero, nav, mobile nav, CTA band, use-case hero/mid, pricing plan cards — those carry `plan` + the calculator's `seats`) |
